...
```{r anisofit, cache = TRUE}
details <- list(distribution = 'binomial', userdist = anisodistfn, 
                miscparm = c(psiA = 0.5, psiR = 1.5))  # initial values
fit <- secr.fit(CH, buffer = 200, detectfn = 'HHN', details = details, trace = FALSE)
coef(fit)
```

The estimated bearing is `r round(coef(fit)['psiA','beta'],2)`. The estimated aspect ratio is `r round(1 + exp(coef(fit)['psiR', 'beta']),2)`, (95% CI `r round(1 + exp(coef(fit)['psiR', c('lcl','ucl')]), 2)`).
...
##temporary
simrandom100.3 <- simrandom100.2
simrandomBVN100.3 <- simrandomBVN100.2
...
## Main simulations 36-detector linear array 50-m spacing
...
simsum(list(Parallel      = simgridalignsqw1.1,
             Oblique       = simgridalignsqw2.1,
             Random        = simgridalignsqw3.1))
```

## Linear with 25 m spacing
...
## Spacing effect on linear arrays
...
simplot(list(Parallel      = simgridalignsq1.2[1:4],
        ...
        legend = TRUE, ylim = c(-1,3.5))
mtext(side=3, text = '25 m')
simplot(list(Parallel      = simgridalignsqw1.1[1:4],
...
```{r sumplotcommon, fig.width = 8, fig.height = 4}
...
Using function `anisodistfn` defined previously. Parameters `psiA` and `psiR` are silently passed to `anisodistfn` in the 'miscparm' mask attribute. Extract the fitted coefficients corresponding to `psiA` and `psiR` with the `coef` method for secr objects, and rely on direct estimation of density (CL = FALSE).
...
baseargs <- list(nrepl = nrepl, buffer = 200, ncores = ncores, traps = tr, 
                 ...
                 details = details, extractfn = coef)
for (i in 1:4) {
    sigmaX <- 25/i^0.5; sigmaY <- 25*i^0.5
    args <- c(basearg, list(sigmaX = sigmaX, sigmaY = sigmaY))
    # bvn parallel to traps
    args$theta <- 0
