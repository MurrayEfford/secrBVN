...
In principle, we can deal with uniformly oriented and elongated ranges by replacing Euclidean distances with distances in a transformed space (Murphy et al. 2016). A simple transformation uses two parameters: $\psi_A$ for the orientation and $\psi_R$ for the degree of inflation or contraction. Thanks to Ben Augustine for pointing out the **geoR** function `coords.aniso` that lets us do this (Ribeiro and Diggle 2018).

The method does not work if the detector array is strictly linear (2-D data are required to estimate elongation). It is tested below on a hollow square array. In this version both transformation parameters are estimated (cf Murphy et al. 2016). This is the only place in this vignette that we attempt to *fit* elongated ranges rather than circular ranges.
...
simplot(list(Parallel = simgridalignsqw1.1,
             Oblique = simgridalignsqw2.2, 
             Random = simgridalignsqw3.2),
        ylim = c(-0.4,0.4), legend = TRUE)
```
