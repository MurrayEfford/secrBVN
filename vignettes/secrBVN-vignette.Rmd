...
This small package evaluates SECR when home ranges are BVN or uniform (flat-topped) ellipses. Code to use **secrBVN** for the simulations of Efford (in prep.) is provided in the [Appendix](#Appendix).

The key user-visible functions are `simpopn.bvn`, `plotpopn.bvn`, `simcapt.bvn`, `runEllipseSim` and `sumplot`.
...
The preceding paragraph describes the default behaviour of `simcapt.bvn`. If `type = uniform` is selected then a uniform (flat-topped) elliptical home range is simulated. The uniform probability of detection within the ellipse is controlled by `g0`.
...
The package function `sumplot` is used to summarize the results
...
output <- sumplot(list(Uniform = simrandom36.2[1:4], BVN = simrandomBVN36.2[1:4]))
```
...
par(mfrow = c(1,1), mar = c(4,4,4,4), xpd = FALSE)
sumplot(list(Uniform = simcirc, BVN = simcircBVN), plt = FALSE)
```
...
ncores <- 20
library(geoR)
tr <- make.grid(36, 1, spacex = 25, spacey = 25, detector = 'proximity')
simaniso1 <- vector('list', 4); names(simaniso1) <- 1:4
simaniso3 <- simaniso2 <- simaniso1
for (i in 1:4) {
    sigmaX <- 25/i^0.5; sigmaY <- 25*i^0.5
    ...
    simaniso2[[i]] <- runEllipseSim (nrepl, sigmaX, sigmaY, buffer = 200, 
        ncores = ncores, traps = tr, lambda0 = 0.4, D = 4, type = 'BVN', 
        details = details, theta = pi/4) 
    # bvn perpendicular to traps
    simaniso3[[i]] <- runEllipseSim (nrepl, sigmaX, sigmaY, buffer = 200, 
        ncores = ncores, traps = tr, lambda0 = 0.4, D = 4, type = 'BVN', 
        details = details, theta = pi/2) 
}
save(simaniso1, file = paste0(vignettefolder, 'simaniso1.RData'))
save(simaniso2, file = paste0(vignettefolder, 'simaniso2.RData'))
save(simaniso3, file = paste0(vignettefolder, 'simaniso3.RData'))
```

```(r sumplotaniso)
load(file = paste0(vignettefolder, 'simaniso1.RData'))
load(file = paste0(vignettefolder, 'simaniso2.RData'))
load(file = paste0(vignettefolder, 'simaniso3.RData'))
sumplot(list(Parallel = simaniso1, 
             Oblique = simaniso2, 
             Perpendicular = simaniso3),
             plt = FALSE)
```
...
ncores <- 20
runsim <- FALSE
details <- list(distribution = 'binomial') # all simulations fixed-N
```
...
sumplot(list(Uniform = simrandom100.2[1:4], BVN = simrandomBVN100.2[1:4]), legend = TRUE)
# tabulate all
sumplot(list(Uniform = simrandom100.2, BVN = simrandomBVN100.2), compact = NULL, 
    plt = FALSE, dec = 4)
```
...
sumplot(list(Uniform = simrandom100.2, BVN = simrandomBVN100.2), plt = FALSE)
# spatial scale parameter
output <- sumplot(list(Uniform = simrandom100.2, BVN = simrandomBVN100.2), plt = FALSE,
        compact= c('av.sighat', 'se.sighat'))
...
tr <- make.grid(10, 10, spacex = 25, spacey = 25, detector = 'proximity', hollow = TRUE)
...
sumplot(list(Parallel = simgridalignsq1.2[1:4],
             Oblique = simgridalignsq2.2[1:4], 
             Random = simgridalignsq3.2[1:4]),
        plt = TRUE, ylim = c(-0.4,0.4), legend = TRUE)
sumplot(list(Parallel = simgridalignsq1.2,
...
par(mfrow = c(1,1), mar = c(4,4,4,4), xpd = FALSE)
sumplot(list(Uniform = simrandom100low.2, BVN = simrandomBVN100low.2), 
    trueD = 0.5, legend = TRUE)
sumplot(list(Uniform = simrandom100low.2, BVN = simrandomBVN100low.2), 
    trueD = 0.5, plt = FALSE, compact = NULL)
```
...
sumplot(list(Uniform = simrandom36.2[1:4], BVN = simrandomBVN36.2[1:4]), legend = TRUE)
sumplot(list(Uniform = simrandom36.2, BVN = simrandomBVN36.2), plt = FALSE)
```
...
sumplot(list(Uniform = simrandom100C.2, BVN = simrandomBVN100C.2), legend = TRUE)
```

# Old appendix 2018-08-04
...
```{r gridalignment, eval = runall}
nrepl <- 500
tr <- make.grid(3,11, spacex = 125, spacey = 25, detector = 'proximity')
simgridalign1 <- vector('list', 4); names(simgridalign1) <- 1:4
simgridalign2 <- simgridalign1
for (i in 1:4) {
    sigmaX <- 25/i^0.5; sigmaY <- 25*i^0.5
    ## bvn parallel to traps
    simgridalign1[[i]] <- runEllipseSim (nrepl, sigmaX, sigmaY, buffer = 200, ncores = ncores, 
                               traps = tr, lambda0 = 0.4, D = 4, type = 'BVN', CL = TRUE, 
                               details = details, theta = 0) 
    ## bvn perpendicular to traps
    simgridalign2[[i]] <- runEllipseSim (nrepl, sigmaX, sigmaY, buffer = 200, ncores = ncores, 
                               traps = tr, lambda0 = 0.4, D = 4, type = 'BVN', CL = TRUE, 
                               details = details, theta = pi/2) 
}
save(simgridalign1, file = paste0(vignettefolder, 'simgridalign1.RData'))
save(simgridalign2, file = paste0(vignettefolder, 'simgridalign2.RData'))
```
```{r, results="hold"}
load(file = paste0(vignettefolder, 'simgridalign1.RData'))
load(file = paste0(vignettefolder, 'simgridalign2.RData'))
```
...
par(mfrow=c(1,1), mar=c(4,4,4,4), xpd = FALSE)
sumplot(list(Parallel = simgridalign1, Perpendicular = simgridalign2), ylim = c(-0.6,0.3))
```
